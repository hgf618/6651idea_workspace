 ...
 * 房屋信息连接数据库
 */
public class FwxxDaoImpl2 implements FwxxDao {
    ...
    public int findCountByCon(FwxxCon fwxxCon){
       ...
       sql+=getSqlCon(fwxxCon);//添加条件字符串
        System.out.println(sql+"=======");
        return ConnectionManager.executeCount(sql);
     ...
     * @return
     */
    private String getSqlCon(FwxxCon fwxxCon){
        String sqlCon="";
        if(fwxxCon!=null){
            if(fwxxCon.getFwxx()!=null){
                Fwxx fwxx=fwxxCon.getFwxx();
                if(fwxx.getTitle()!=null&&!fwxx.getTitle().equals("")){
                    sqlCon+=" and title  like '%"+fwxx.getTitle() +"%'";
                }
                if(fwxx.getShi()!=0) {
                    sqlCon+=" and shi = "+fwxx.getShi();
                }
                if(fwxx.getTing()!=0) {
                    sqlCon+=" and ting = "+fwxx.getTing();
                }

                if(fwxx.getJd()!=null){
                    if(fwxx.getJd().getJdid()!=0) {
                        sqlCon += " and jdid=" + fwxx.getJd().getJdid();
                    }else if(fwxx.getJd().getQx()!=null&&fwxx.getJd().getQx().getQxid()!=0){
                        sqlCon +=" and jdid in (select jdid from tbl_jd where qxid="+fwxx.getJd().getQx().getQxid()+")";
                    }
                }

                if(fwxx.getUser()!=null&&fwxx.getUser().getUid()!=0){
                    sqlCon+=" and uid="+fwxx.getUser().getUid();
                }
            }
            if(fwxxCon.getZj1()!=0){
                sqlCon+=" and zj>="+fwxxCon.getZj1();
            }
            if(fwxxCon.getZj2()!=0){
                sqlCon+=" and zj<="+fwxxCon.getZj2();
            }
            ...
            if(fwxxCon.getLxids()!=null&&fwxxCon.getLxids().length>0){
                ...
                sqlCon+=" and lxid in ("+lxidStr.substring(0, lxidStr.length()-1)+")";//  4,2,1
            }

            if(fwxxCon.getDays()!=0){
                sqlCon+=" and  datediff(now(),date)<="+fwxxCon.getDays();
            }
        }
        return sqlCon;
    }
